...
class AlunoControllerTest {
    ...
    public void criarAluno_ComDadosValidos_RetornarAlunoComStatus204() throws Exception {
        ...
        testClient.perform(post("/alunos")  // simula uma requisição post
                        .contentType(MediaType.APPLICATION_JSON) // .Define o cabeçalho Content-Type da requisição enviada ao controller.
                        .content(jsonDeEntrada) // Define o corpo (body) da requisição HTTP que será enviada.25250nnbbbb ,,,,4
                       )
                .andExpect(status().isCreated()); //Verifica se o status HTTP da resposta foi o esperado.
        verify(service).salvar(any(Aluno.class)); // Confirma que o metodo salvar() no service foi chamado exatamente uma vez durante a execução da requisição HTTP
    ...
    public void criarAluno_ComDadosInvalidos_RetornarErroMessage400() throws Exception {
        ...
        testClient.perform(post("/alunos")
    ...
    public void alterarAluno_ComIdExistentes_RetornarStatus204() throws Exception {
        ...
        testClient.perform(patch("/alunos/{id}",aluno_id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(jsonDeEntrada)
    ...
    public void atualizarAluno_ComIdInexistentes_RetornarErroMessage404() throws Exception {
        ...
        doThrow(new AlunoNaoEncontradoException(aluno_id)).when(service).atualizarAluno(eq(aluno_id), any(Aluno.class));
    ...
    @Test
    public void buscarAluno_ComIdExistente_RetornarAlunoComStatus200() throws Exception { // Pode melhorar fazendo verificar se o JSON retornado está correto e se o mapeamento para o DTO está funcionando conforme esperado, mas teria que arrumar a classe AlunoDto
        String jsonDeEntrada = "{"
                + "\"nome\": \"KARINE\","
                + "\"sobrenome\": \"FERREIRA\","
                + "\"genero\": \"FEMININO\","
    ...
    @Test
    public void buscarTodosOsAlunos_ComFiltrosValidos_RetornarListaComAlunosComStatus200() throws Exception {
        Integer pagina = 0;
        Integer limite = 10;
        Long aluno_id = 1L;
       ...
       when(service.listarAlunos(any(AlunoFilters.class), eq(pagina), eq(limite))).thenReturn(pageAluno);

       when(alunoMapper.mapearParaAlunoDtoPage(any())).thenReturn(alunosDtoPageMock);

       testClient.perform(get("/alunos")
    ...
    public void buscarTodosOsAlunos_ComFiltrosInvalidos_RetornarErro400() throws Exception {
       ...
       when(service.listarAlunos(any(AlunoFilters.class), eq(pagina), eq(limite))).thenThrow(ParametrosListagemInvalidosException.class);
